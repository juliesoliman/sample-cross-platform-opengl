...
void main()
{
    ...
    color.r = 0.4 * sin( ( (time/2.0) - position.y * 4.0 ) - position.y * 5.0 );
    color.b = 1.0;// * cos( ( time + position.y * 8.0 ) + position.y * 2.0 );
    color.g = 0.8 + 0.2 * sin( ( (time/2.0) - position.y * 4.0 ) - position.y * 5.0 );
    color.a = 1.0;
    
    float zero = 0.0;
    if( color.r < zero ) color.r *= -1.0;
    if( color.g < zero ) color.g *= -1.0;
    if( color.b < 0.6  ) color.b  =  0.60;
}
